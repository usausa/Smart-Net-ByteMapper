namespace Smart.IO.Mapper.Mappers
{
    ...
    public sealed class ShortConvertMapper : IMemberMapper
    {
        ...
        public void Read(byte[] buffer, int index, object target)
        {
            ...
            if ((value.Length > 0) && Int16.TryParse(value, style, info, out var result))
            {
        ...
        public void Write(byte[] buffer, int index, object target)
        {
            ...
            {
                BytesHelper.WriteString(((short)value).ToString(info), buffer, index + offset, Length, encoding, padding, filler);
            }
        }
    }
}

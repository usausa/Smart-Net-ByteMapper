namespace Smart.IO.ByteMapper.Helpers
{
    ...
    public class NumberHelperIntegerTest
    {
        [Fact]
        public void ParseInteger()
        {
            ...
            Assert.True(NumberHelper.TryParseInt64(buffer, 0, buffer.Length, 0x20, out var value));
            Assert.Equal(1234567890123456789L, value);

            // Negative
            buffer = Encoding.ASCII.GetBytes("-1234567890123456789");
            Assert.True(NumberHelper.TryParseInt64(buffer, 0, buffer.Length, 0x20, out value));
            Assert.Equal(-1234567890123456789L, value);
            ...
            Assert.True(NumberHelper.TryParseInt16(buffer, 0, buffer.Length, 0x20, out var shortValue));
            ...
            Assert.True(NumberHelper.TryParseInt16(buffer, 0, buffer.Length, 0x20, out shortValue));
            ...
            Assert.True(NumberHelper.TryParseInt32(buffer, 0, buffer.Length, 0x20, out var intValue));
            Assert.Equal(Int32.MaxValue, intValue);

            buffer = Encoding.ASCII.GetBytes(Int32.MinValue.ToString(CultureInfo.InvariantCulture));
            Assert.True(NumberHelper.TryParseInt32(buffer, 0, buffer.Length, 0x20, out intValue));
            ...
            Assert.False(NumberHelper.TryParseInt64(buffer, 0, buffer.Length, 0x20, out value));
        ...
        public void FormatInteger()
        {
            ...
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, 1234567890123456789L, Padding.Left, false, 0x20);
            Assert.Equal("   1234567890123456789", Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, 1234567890123456789L, Padding.Left, true, 0x20);
            Assert.Equal("0001234567890123456789", Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, 1234567890123456789L, Padding.Right, false, 0x20);
            Assert.Equal("1234567890123456789   ", Encoding.ASCII.GetString(buffer, 0, buffer.Length));
            ...
            // Max value
            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, Int64.MaxValue, Padding.Left, false, 0x20);
            Assert.Equal(String.Format("{0,22}", Int64.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, Int64.MaxValue, Padding.Left, true, 0x20);
            Assert.Equal(String.Format("{0,22:D22}", Int64.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, Int64.MaxValue, Padding.Right, false, 0x20);
            Assert.Equal(String.Format("{0,-22}", Int64.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            // Min value
            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, Int64.MinValue, Padding.Left, false, 0x20);
            Assert.Equal(String.Format("{0,22}", Int64.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, Int64.MinValue, Padding.Left, true, 0x20);
            Assert.Equal(String.Format("{0,22:D21}", Int64.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[22];
            NumberHelper.FormatInt64(buffer, 0, buffer.Length, Int64.MinValue, Padding.Right, false, 0x20);
            Assert.Equal(String.Format("{0,-22}", Int64.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));
            ...
            // Max value
            buffer = new byte[9];
            NumberHelper.FormatInt16(buffer, 0, buffer.Length, Int16.MaxValue, Padding.Left, false, 0x20);
            Assert.Equal(String.Format("{0,9}", Int16.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[9];
            NumberHelper.FormatInt16(buffer, 0, buffer.Length, Int16.MaxValue, Padding.Left, true, 0x20);
            Assert.Equal(String.Format("{0,9:D9}", Int16.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[9];
            NumberHelper.FormatInt16(buffer, 0, buffer.Length, Int16.MaxValue, Padding.Right, false, 0x20);
            Assert.Equal(String.Format("{0,-9}", Int16.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            // Min value
            buffer = new byte[9];
            NumberHelper.FormatInt16(buffer, 0, buffer.Length, Int16.MinValue, Padding.Left, false, 0x20);
            Assert.Equal(String.Format("{0,9}", Int16.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[9];
            NumberHelper.FormatInt16(buffer, 0, buffer.Length, Int16.MinValue, Padding.Left, true, 0x20);
            Assert.Equal(String.Format("{0,9:D8}", Int16.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[9];
            NumberHelper.FormatInt16(buffer, 0, buffer.Length, Int16.MinValue, Padding.Right, false, 0x20);
            Assert.Equal(String.Format("{0,-9}", Int16.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));
            ...
            // Max value
            buffer = new byte[12];
            NumberHelper.FormatInt32(buffer, 0, buffer.Length, Int32.MaxValue, Padding.Left, false, 0x20);
            Assert.Equal(String.Format("{0,12}", Int32.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[12];
            NumberHelper.FormatInt32(buffer, 0, buffer.Length, Int32.MaxValue, Padding.Left, true, 0x20);
            Assert.Equal(String.Format("{0,12:D12}", Int32.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[12];
            NumberHelper.FormatInt32(buffer, 0, buffer.Length, Int32.MaxValue, Padding.Right, false, 0x20);
            Assert.Equal(String.Format("{0,-12}", Int32.MaxValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            // Min value
            buffer = new byte[12];
            NumberHelper.FormatInt32(buffer, 0, buffer.Length, Int32.MinValue, Padding.Left, false, 0x20);
            Assert.Equal(String.Format("{0,12}", Int32.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[12];
            NumberHelper.FormatInt32(buffer, 0, buffer.Length, Int32.MinValue, Padding.Left, true, 0x20);
            Assert.Equal(String.Format("{0,12:D11}", Int32.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));

            buffer = new byte[12];
            NumberHelper.FormatInt32(buffer, 0, buffer.Length, Int32.MinValue, Padding.Right, false, 0x20);
            Assert.Equal(String.Format("{0,-12}", Int32.MinValue), Encoding.ASCII.GetString(buffer, 0, buffer.Length));
        }
    }
}
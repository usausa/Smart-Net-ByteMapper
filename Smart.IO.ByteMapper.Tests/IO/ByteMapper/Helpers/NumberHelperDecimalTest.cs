namespace Smart.IO.ByteMapper.Helpers
{
    ...
    public class NumberHelperDecimalTest
    {
        ...
        public void ParseDecimal()
        {
            ...
            Assert.True(NumberHelper.TryParseDecimal(buffer, 0, buffer.Length, 0x20, out var value));
            Assert.Equal(1234567890123456.78m, value);

            // Negative
            buffer = Encoding.ASCII.GetBytes("-1,234,567,890,123,456.78");
            Assert.True(NumberHelper.TryParseDecimal(buffer, 0, buffer.Length, 0x20, out value));
            Assert.Equal(-1234567890123456.78m, value);
        ...
        //    NumberHelper.TryParseDecimal(bytes, 0, bytes.Length, (byte)' ', out var value);

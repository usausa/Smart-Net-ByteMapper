...
public sealed class CoverageFixTest
{
    ...
    {
        Assert.Throws<NotSupportedException>(() => new ConstantMapper(0, []).Also(x => x.Read([], null)));
        Assert.Throws<NotSupportedException>(() => new FillMapper(0, 0, 0x00).Also(x => x.Read([], null)));
    }
}
